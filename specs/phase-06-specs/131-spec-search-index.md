...
Implement a search indexing system for specifications that enables fast, full-text search with faceted filtering. The index supports fuzzy matching, field-specific queries, and relevance ranking optimized for finding specs by various criteria.
...
impl IndexBuilder {
    ...
    pub fn from_parsed(
        parsed: &crate::spec::parsing::ParsedSpec,
        path: &Path,
    ) -> IndexedSpec {
        // Combine all content
...
- [ ] Tests for term search
- [ ] Tests for prefix search
- [ ] Tests for facet filtering
- [ ] Tests for index persistence
- [ ] Performance tests for large indexes

## Related Specs

- **132-spec-search-api.md**: Search API using this index
- **120-spec-parsing.md**: Parsing for index content
- **121-spec-metadata.md**: Metadata for facets
