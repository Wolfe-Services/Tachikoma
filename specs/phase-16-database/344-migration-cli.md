...
```rust
// src/bin/tachikoma.rs (partial)

use tachikoma::database::migration::cli::MigrateCli;
...
match cli.command {
    Commands::Migrate(cmd) => {
        let migrate_cli = MigrateCli { command: cmd, ..Default::default() };
        migrate_cli.run().await?;
    }
}
```
...
```bash
# Create a new migration
tachikoma migrate create "add_users_table"

# Run all pending migrations
tachikoma migrate up

# Run up to specific version
tachikoma migrate up --to 20240101120000

# Rollback last migration
tachikoma migrate down

# Rollback last 3 migrations
tachikoma migrate down 3

# Rollback to specific version
tachikoma migrate down --to 20240101000000

# Show migration status
tachikoma migrate status

# Verify checksums
tachikoma migrate verify

# Reset database (dangerous!)
tachikoma migrate reset --force
```

## Files to Create
- `src/database/migration/cli.rs` - CLI implementation
