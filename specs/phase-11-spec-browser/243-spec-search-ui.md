...
Create a search interface for finding specifications by title, content, tags, or ID with real-time results, search history, and advanced filters.
...
### 1. Types (src/lib/types/spec-search.ts)
...
### 2. Spec Search UI Component (src/lib/components/spec-browser/SpecSearchUI.svelte)
...
<script lang="ts">
  ...
  async function search() {
    ...
    isSearching = true;
    try {
      results = await ipcRenderer.invoke('spec:search', { text: query, filters });
      selectedIndex = 0;
...
- Next: [244-spec-search-results.md](244-spec-search-results.md)
