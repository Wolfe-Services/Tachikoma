# Spec 551: Application Shell Integration
...
- [ ] Replace placeholder `+page.svelte` with Mission Panel layout
- [ ] Add sidebar navigation with routes (Mission, Specs, Forge, Settings, Dashboard)
- [ ] Wire up existing components: `MissionComparison`, `ContextMeter`, `CostTracking`, etc.
- [ ] Implement responsive layout with collapsible sidebar
- [ ] Add header with app title, status indicator, and user menu
- [ ] Fix the `crypto is not defined` error in mission:start handler
- [ ] Verify all IPC handlers work with real UI interactions
