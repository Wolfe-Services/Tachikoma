# Spec 553: Quickstart Onboarding
...
- [ ] Create `tachikoma init` CLI command
- [ ] Auto-detect installed tools (cargo, node, jj, git)
- [ ] Auto-detect API keys from environment or known locations
- [ ] Create minimal `.tachikoma/config.yaml` with sane defaults
- [ ] Create sample `specs/` directory with getting-started spec
- [ ] Print clear next steps
- [ ] Support `--quick` flag for non-interactive mode
- [ ] Add `tachikoma doctor` to diagnose setup issues
...
Creating configuration...
  ✓ Created .tachikoma/config.yaml
  ✓ Created specs/README.md
  ✓ Created specs/001-getting-started.md
...
🕷️ Tachikoma Health Check

Environment:
  ✓ Rust 1.82.0
  ✓ Node.js 22.0.0
  ✓ jj 0.24.0
  ✗ ripgrep not found (code_search won't work)
...
Configuration:
  ✓ .tachikoma/config.yaml exists
  ✓ Backend 'claude' is available
  ✓ Specs directory found (546 specs)
...
- [ ] Run `tachikoma run --spec 001` to let Tachikoma implement something simple
- [ ] Watch the loop execute and learn the pattern
- [ ] Create your first custom spec with `tachikoma chat`
