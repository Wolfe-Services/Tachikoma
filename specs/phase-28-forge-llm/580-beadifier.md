...
- [ ] Create `crates/tachikoma-forge/src/output/beadifier.rs`
- [ ] Define `BeadTask` struct: title, description, priority (P0-P4), dependencies, type (task/bug/feature)
- [ ] Define `BeadifyConfig`: max_tasks, target (Beads | SpecFiles), epic_id (for hierarchy)
- [ ] Implement `Beadifier::extract_tasks(summary: &ConsensusSummary, llm: &dyn LlmProvider) -> Vec<BeadTask>`
- [ ] Use iterative prompting: "Given this decision, what is the FIRST atomic task?"
- [ ] Validate each task: title < 80 chars, description < 200 chars, no compound verbs ("and", "then")
- [ ] Implement `Beadifier::to_beads(tasks: &[BeadTask]) -> Vec<String>` returning `bd create` commands
- [ ] Implement `Beadifier::to_spec_files(tasks: &[BeadTask], dir: &Path) -> Result<()>` writing markdown files
