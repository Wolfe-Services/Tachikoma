...
mod tests {
    ...
    fn test_color_mode_conversion() {
        assert_eq!(ColorMode::from(clap::ColorChoice::Auto), ColorMode::Auto);
        assert_eq!(ColorMode::from(clap::ColorChoice::Always), ColorMode::Always);
        assert_eq!(ColorMode::from(clap::ColorChoice::Never), ColorMode::Never);
    }

    #[test]
    fn test_color_depth_detection() {
        // This test may vary based on environment, so just ensure it returns a valid value
        let depth = ColorDepth::detect();
        assert!(matches!(depth, ColorDepth::None | ColorDepth::Basic | ColorDepth::Extended | ColorDepth::TrueColor));
    }
    ...
    fn test_semantic_colors() {
        let styled = Styled::new("test")
            .with_color_mode(ColorMode::Always)
            .success();
        assert!(styled.to_string().contains("\x1b[32m")); // Green
        
        let styled = Styled::new("test")
            .with_color_mode(ColorMode::Always)
            .error();
        assert!(styled.to_string().contains("\x1b[31m")); // Red
    }
}