...
mod tests {
    use super::*;
    use crate::parsing::{ParsedSpec, SpecMetadata as ParsedMetadata, LineMap, Checkbox};
    use std::collections::HashMap;

    fn create_test_spec() -> ParsedSpec {
        let mut sections = HashMap::new();
        sections.insert("Metadata".to_string(), "metadata content".to_string());
        sections.insert("Objective".to_string(), "objective content".to_string());
        sections.insert("Acceptance Criteria".to_string(), "criteria content".to_string());

        ParsedSpec {
            title: "Spec 127: Test Spec".to_string(),
            ...
            sections,
            section_order: vec!["Metadata".to_string(), "Objective".to_string(), "Acceptance Criteria".to_string()],
            acceptance_criteria: vec![
                Checkbox { text: "Test criterion".to_string(), checked: false, line: 10, section: "Acceptance Criteria".to_string() }
            ],
            code_blocks: Vec::new(),
            references: Vec::new(),
            warnings: Vec::new(),
    ...
    fn create_test_metadata() -> SpecMetadata {
        use crate::metadata::*;
    ...
    fn test_required_sections_rule() {
        let rule = RequiredSectionsRule::new(vec!["Missing Section".to_string()]);
        let spec = create_test_spec();
        let metadata = create_test_metadata();

        let issues = rule.validate(&spec, &metadata);
        assert!(!issues.is_empty());
        assert!(issues[0].message.contains("Missing Section"));
        assert_eq!(issues[0].code, "SPEC001");
    }

    #[test]
    fn test_required_metadata_rule() {
        let rule = RequiredMetadataRule::new(vec!["Missing Field".to_string()]);
        let spec = create_test_spec();
        let metadata = create_test_metadata();
        ...
        assert_eq!(issues[0].code, "SPEC002");
    }

    #[test]
    fn test_acceptance_criteria_rule() {
        let rule = AcceptanceCriteriaRule::new(5); // Require 5 criteria
        let spec = create_test_spec(); // Only has 1
        let metadata = create_test_metadata();

        let issues = rule.validate(&spec, &metadata);
        assert!(!issues.is_empty());
        assert!(issues[0].message.contains("Insufficient acceptance criteria"));
        assert_eq!(issues[0].code, "SPEC003");
    }

    #[test]
    fn test_title_format_rule() {
        let rule = TitleFormatRule;
        let mut spec = create_test_spec();
        spec.title = "Invalid Title Format".to_string();
        let metadata = create_test_metadata();

        let issues = rule.validate(&spec, &metadata);
        assert!(!issues.is_empty());
        assert!(issues[0].message.contains("Title doesn't match expected format"));
        assert_eq!(issues[0].code, "SPEC010");
    }

    #[test]
    fn test_status_value_rule() {
        let rule = StatusValueRule;
        let mut spec = create_test_spec();
        spec.metadata.status = "Invalid Status".to_string();
        let metadata = create_test_metadata();

        let issues = rule.validate(&spec, &metadata);
        assert!(!issues.is_empty());
        assert!(issues[0].message.contains("Invalid status value"));
        assert_eq!(issues[0].code, "SPEC011");
    }

    #[test]
    fn test_spec_validator_integration() {
        ...
        let result = validator.validate(&spec, &metadata);
        assert_eq!(result.spec_id, 127);
        assert_eq!(result.path, "test.md");
        
        // Should have some issues since we don't have Implementation/Test sections
        assert!(!result.issues.is_empty());
        
        // Check counts
        assert!(result.counts.warnings > 0 || result.counts.errors > 0 || result.counts.infos > 0);
    }
    ...
    fn test_batch_validation() {
        ...
        spec2.metadata.spec_id = 128;
        let mut metadata2 = create_test_metadata();
        metadata2.id = 128;

        let specs = vec![(spec1, metadata1), (spec2, metadata2)];
        let results = validator.validate_batch(&specs);
        
        assert_eq!(results.len(), 2);
        assert_eq!(results[0].spec_id, 127);
        assert_eq!(results[1].spec_id, 128);
    }

    #[test]
    fn test_severity_overrides() {
        let mut config = ValidationConfig::default();
        config.severity_overrides.insert("title-format".to_string(), Severity::Error);
        
        let validator = SpecValidator::new(config);
        let mut spec = create_test_spec();
        spec.title = "Bad Title".to_string();
        let metadata = create_test_metadata();

        let result = validator.validate(&spec, &metadata);
        
        // Find the title format issue and verify severity was overridden
        let title_issue = result.issues.iter()
            .find(|i| i.rule == "title-format")
            .expect("Should have title format issue");
    ...
    fn test_disabled_rules() {
        let mut config = ValidationConfig::default();
        config.disabled_rules.insert("title-format".to_string());
        ...
        // Should not have any title format issues
        let has_title_issue = result.issues.iter()
            .any(|i| i.rule == "title-format");
