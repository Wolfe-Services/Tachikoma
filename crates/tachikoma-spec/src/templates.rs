...
impl TemplateEngine {
    ...
    /// Validate template context against requirements
    pub fn validate_context(
        ...
        let template = self.templates.get(&template_type)
            .ok_or(TemplateError::NotFound(template_type.filename().to_string()))?;

        // Check required variables
        for var in &template.variables {
            if var.required && !context.custom.contains_key(&var.name) {
                // Check built-in context fields
                match var.name.as_str() {
                    "spec_id" | "title" | "phase" | "phase_name" | "slug" 
                    | "dependencies" | "estimated_context" => continue,
                    _ => return Err(TemplateError::MissingVariable(var.name.clone())),
                }
            }
        }
    ...
    pub async fn create_spec(
        ...
        self.validate_context(template_type, context)?;
    ...
    /// Get template info
    pub fn get_template(&self, template_type: TemplateType) -> Option<&SpecTemplate> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_all_template_types() {
        let temp = TempDir::new().unwrap();
        let engine = TemplateEngine::new(temp.path()).await.unwrap();
        ...
        let context = TemplateContext {
            ...
            estimated_context: "~5%".to_string(),
            custom: HashMap::new(),
        };

        for template_type in TemplateType::all() {
            let result = engine.render(*template_type, &context).unwrap();
            assert!(result.contains("Spec 100"));
    ...
    #[tokio::test]
    async fn test_create_spec_file() {
        let temp = TempDir::new().unwrap();
        let engine = TemplateEngine::new(temp.path()).await.unwrap();
        ...
        let output_path = temp.path().join("test-spec.md");
        let result_path = engine.create_spec(TemplateType::Feature, &context, &output_path).await.unwrap();

        assert_eq!(result_path, output_path);
        assert!(output_path.exists());

        let content = fs::read_to_string(&output_path).await.unwrap();
        assert!(content.contains("Spec 100"));
    }
}