...
impl ReadmeLookup {
    ...
    fn parse_list_entry(&self, line: &str) -> Option<ReadmeSpecEntry> {
        ...
        if let Some(caps) = re.captures(line) {
            ...
            let link = caps.get(2)?.as_str().to_string();
            let status = caps.get(3)
                .map(|m| SpecStatus::from_str(m.as_str()))
                .unwrap_or(SpecStatus::Planned);

            // Extract ID from filename (e.g., "116-spec-directory")
            let id: u32 = filename.split('-').next()?.parse().ok()?;
            let title = filename[3..].replace('-', " "); // Skip "116-" prefix
...
mod tests {
    ...
    fn test_section_detection() {
        ...
        assert_eq!(lookup.detect_section("## Overview"), Some(ReadmeSection::Overview));
        assert_eq!(lookup.detect_section("## Specifications"), Some(ReadmeSection::SpecList));
        assert_eq!(lookup.detect_section("## Progress"), Some(ReadmeSection::Progress));
        assert_eq!(lookup.detect_section("## Dependencies"), Some(ReadmeSection::Dependencies));
        assert_eq!(lookup.detect_section("## Quick Reference"), Some(ReadmeSection::QuickReference));
        assert_eq!(lookup.detect_section("## Notes"), Some(ReadmeSection::Notes));
        assert_eq!(lookup.detect_section("# Not a section"), None);
    }
    ...
    fn test_table_row_parsing() {
        ...
        let table_row = "| 116 | Spec Directory | In Progress | 25% | Directory structure management |";
        let spec = lookup.parse_table_row(table_row).unwrap();

        assert_eq!(spec.id, 116);
        assert_eq!(spec.title, "Spec Directory");
        assert_eq!(spec.status, SpecStatus::InProgress);
        assert_eq!(spec.completion, 25);
        assert_eq!(spec.description.as_ref().unwrap(), "Directory structure management");
    }
    ...
    fn test_list_entry_parsing() {
        ...
        let list_entry = "- [116-spec-directory.md](./116-spec-directory.md) - In Progress";
        let spec = lookup.parse_list_entry(list_entry).unwrap();

        assert_eq!(spec.id, 116);
        assert_eq!(spec.title, "spec-directory");
        assert_eq!(spec.status, SpecStatus::InProgress);
        assert_eq!(spec.link, "./116-spec-directory.md");
    }
    ...
    fn test_dependencies_parsing() {
        ...
        let content = "This phase depends on Phase 1 and Phase 3 for completion.";
        let deps = lookup.parse_dependencies(content);

        assert_eq!(deps, vec![1, 3]);
    }

    #[test]
    fn test_quick_reference_parsing() {
        let lookup = ReadmeLookup::new(PathBuf::new());

        let content = r#"
- **Core Component**: Directory structure
- **Main API**: SpecDirectory
- **Testing**: Unit tests included
"#;
        let items = lookup.parse_quick_reference(content);

        assert_eq!(items.len(), 3);
        assert_eq!(items[0].label, "Core Component");
        assert_eq!(items[0].value, "Directory structure");
        assert_eq!(items[1].label, "Main API");
        assert_eq!(items[1].value, "SpecDirectory");
    }

    #[test]
    fn test_readme_template() {
        let template = readme_template(6, "Spec System");

        assert!(template.contains("Phase 6: Spec System"));
        assert!(template.contains("## Overview"));
        assert!(template.contains("## Specifications"));
