...
use crate::metadata::{SpecMetadata, SpecStatus};
use crate::checkbox::{CheckboxTracker, CheckboxStats};
...
mod tests {
    use super::*;
    use crate::metadata::{AcceptanceCriteriaStats, ImplementationStatus, ContextEstimate, Confidence};
    ...
    fn create_mock_checkbox_stats() -> CheckboxStats {
        let mut by_section = HashMap::new();
        by_section.insert("Acceptance Criteria".to_string(), (10, 7));
        by_section.insert("Testing Requirements".to_string(), (5, 2));
    ...
    fn test_progress_calculation() {
        ...
        let metadata = create_mock_metadata(1);
        let stats = create_mock_checkbox_stats();

        let progress = calc.calculate_spec_progress(&metadata, &stats);
        assert!(progress.overall > 0);
        assert_eq!(progress.spec_id, 1);
        assert!(progress.components.acceptance_criteria.percentage > 0);
        assert!(progress.components.implementation.percentage > 0);
    }

    #[test]
    fn test_component_weights() {
        let calc = ProgressCalculator::default();
        let progress_comp = calc.calc_acceptance_progress(&create_mock_checkbox_stats());

        assert_eq!(progress_comp.percentage, 60);
        assert_eq!(progress_comp.total, 15);
        assert_eq!(progress_comp.completed, 9);
        assert_eq!(progress_comp.weight, 0.4);
    }

    #[test]
    fn test_implementation_progress() {
        let calc = ProgressCalculator::default();
        let mut metadata = create_mock_metadata(1);
        
        // Test both implementation and code
        metadata.implementation_status.has_implementation = true;
        metadata.implementation_status.has_code = true;
        
        let impl_progress = calc.calc_implementation_progress(&metadata);
        assert_eq!(impl_progress.percentage, 100);
        assert_eq!(impl_progress.completed, 2);

        // Test only implementation
        metadata.implementation_status.has_code = false;
        let impl_progress = calc.calc_implementation_progress(&metadata);
        assert_eq!(impl_progress.percentage, 50);
        assert_eq!(impl_progress.completed, 1);
    }
    ...
    fn test_phase_aggregation() {
        ...
        let specs = vec![
            (create_mock_metadata(1), create_mock_checkbox_stats()),
            (create_mock_metadata(2), create_mock_checkbox_stats()),
        ];

        let phase_progress = calc.calculate_phase_progress(1, "Test Phase", specs);
        
        assert_eq!(phase_progress.phase, 1);
        assert_eq!(phase_progress.name, "Test Phase");
        assert_eq!(phase_progress.spec_counts.total, 2);
        assert_eq!(phase_progress.specs.len(), 2);
        assert!(phase_progress.avg_spec_progress > 0.0);
    }

    #[test]
    fn test_project_rollup() {
        let mut calc = ProgressCalculator::default();
        ...
        let project_progress = calc.calculate_project_progress(phases);
        
        assert_eq!(project_progress.total_specs.total, 18);
        assert_eq!(project_progress.total_specs.complete, 11);
        assert!(project_progress.overall > 0);
        assert_eq!(project_progress.phases.len(), 2);
    }

    #[test]
    fn test_weighted_progress_calculations() {
        let mut config = ProgressConfig::default();
        config.acceptance_weight = 0.5;
        config.implementation_weight = 0.3;
        config.testing_weight = 0.1;
        config.documentation_weight = 0.1;

        let calc = ProgressCalculator::new(config);
        let metadata = create_mock_metadata(1);
        let stats = create_mock_checkbox_stats();
        ...
        assert!(progress.weighted_score > 0.0);
        assert!(progress.overall > 0);
    }
    ...
    fn test_velocity_calculation() {
        ...
        // Add some history
        calc.history.push(ProgressSnapshot {
        ...
        let velocity = calc.calculate_velocity();
        assert!(velocity > 0.0);
    }
    ...
    fn test_progress_bar_rendering() {
        ...
        assert!(bar.contains("██████████"));
        assert!(bar.contains("░░░░░░░░░░"));
        assert_eq!(bar.len(), 22); // [20 chars]

        let bar_full = calc.render_progress_bar(100);
        assert_eq!(bar_full, "[████████████████████]");

        let bar_empty = calc.render_progress_bar(0);
        assert_eq!(bar_empty, "[░░░░░░░░░░░░░░░░░░░░]");
    }
    ...
    fn test_burn_rate_calculation() {
        ...
        assert_eq!(burn_rate.completed + burn_rate.remaining, burn_rate.projected_total);
        assert!(burn_rate.on_track);
    }

    #[test]
    fn test_progress_trend_detection() {
        let calc = ProgressCalculator::default();
        let mut metadata = create_mock_metadata(1);
        let stats = create_mock_checkbox_stats();

        // Test completed spec
        metadata.status = SpecStatus::Complete;
        let progress = calc.calculate_spec_progress(&metadata, &stats);
        assert_eq!(progress.trend, ProgressTrend::Stable);

        // Test high progress spec
        metadata.status = SpecStatus::InProgress;
        let progress = calc.calculate_spec_progress(&metadata, &stats);
        // This depends on the weighted calculation, but should be either Improving or New
        assert!(matches!(progress.trend, ProgressTrend::Improving | ProgressTrend::New));
    }

    #[test]
    fn test_testing_progress_calculation() {
        let calc = ProgressCalculator::default();
        let metadata = create_mock_metadata(1);
        let stats = create_mock_checkbox_stats();

        let testing_progress = calc.calc_testing_progress(&metadata, &stats);
        
        // Should calculate based on testing checkboxes
        assert!(testing_progress.percentage > 0);
        assert_eq!(testing_progress.total, 5); // From mock stats
        assert_eq!(testing_progress.completed, 2); // From mock stats
    }

    #[test]
    fn test_documentation_progress() {
        let calc = ProgressCalculator::default();
        let metadata = create_mock_metadata(1);

        let doc_progress = calc.calc_documentation_progress(&metadata);
        
        // Should be 100% because mock has section_count=4 and word_count=500
        assert_eq!(doc_progress.percentage, 100);
        assert_eq!(doc_progress.total, 1);
        assert_eq!(doc_progress.completed, 1);
    }
    ...
    fn test_report_generation() {
        ...
        let project_progress = ProjectProgress {
            ...
            phases: vec![
                PhaseProgress {
                    ...
                    avg_spec_progress: 80.0,
                    specs: Vec::new(),
                    blockers: 0,
                    estimated_completion: None,
                },
            ],
            total_specs: SpecCounts {
                total: 10,
                complete: 8,
                in_progress: 2,
                planned: 0,
        ...
        let report = calc.generate_report(&project_progress);
        
        assert!(report.contains("# Progress Report"));
        assert!(report.contains("**65%**"));
        assert!(report.contains("Phase 1: Foundation"));
        assert!(report.contains("Velocity: 2.5"));
        assert!(report.contains("On Track: Yes"));
    }
}