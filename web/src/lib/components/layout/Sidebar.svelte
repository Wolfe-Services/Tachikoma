      ...
      >
        <div class="nav-icon">
          <Icon
            name={item.icon}
            size={20}
            glow={isActiveRoute(item.href, $page.url.pathname)}
          />
        </div>
        {#if !collapsed}
          <div class="nav-content">
            <span class="nav-label"
              >{item.label} <span class="nav-jp">{item.jp}</span></span
            >
...
<style>
  .sidebar {
    ...
    background: var(
      --sidebar-bg,
      linear-gradient(180deg, #0d1117 0%, #0a0c10 100%)
    );
